...
public class CombatLoggerListener implements Listener {
    ...
    public void onEntityDeath(EntityDeathEvent event) {
        ...
        if (hasCombatLoggerMeta) {
            ...
            // Take away DTR.
            if (team != null) {
                team.playerDeath(metadata.playerName, Foxtrot.getInstance().getServerHandler().getDTRLoss(event.getEntity().getLocation()), event.getEntity().getKiller());
            }
...
//              }
    ...
    public void onPlayerQuit(PlayerQuitEvent event) {
        ...
        if (ModUtils.isModMode(event.getPlayer()) || ModUtils.isInvisible(event.getPlayer())) {
            return;
        }
        ...
        if (event.getPlayer().getGameMode() != GameMode.CREATIVE && !ModUtils.isInvisible(event.getPlayer()) && spawnCombatLogger && !event.getPlayer().isDead()) {
            ...
            if (villager.getWorld().getEnvironment() == World.Environment.THE_END) {
                ...
                new BukkitRunnable() {
                    ...
                    public void run() {
                        ...
                        // Take away DTR.
                        if (team != null) {
                            team.playerDeath(metadata.playerName, Foxtrot.getInstance().getServerHandler().getDTRLoss(villager.getLocation()), null);
                        }
