...
public class FFAGame extends Game {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (i == 0) {
                    ...
                    sendMessages(ChatColor.YELLOW + "The FFA is starting in " + ChatColor.DARK_RED + i + ChatColor.YELLOW + " second" + (i == 1 ? "" : "s") + "...");
    ...
    public void getScoreboardLines(Player player, LinkedList<String> lines) {
        if (state == GameState.WAITING) {
            ...
            lines.add("&4Pots: &f" + potions);
            lines.add("&4Kills: &f" + kills.get(player.getUniqueId()));
        } else {
            if (winningPlayer == null) {
