...
public class EOTWCommand {
    ...
    @Command(names = {"eotw invis", "eotw invisibility"}, permission = "foxtrot.eotw")
    public static void eotwInvis(Player sender) {
        ...
        if (!Foxtrot.getInstance().getServerHandler().isEOTW() || !ffaEnabled) {
            sender.sendMessage(ChatColor.RED + "This command must be ran during EOTW FFA. (/eotw ffa)");
            return;
        }

        PotionEffect invis = new PotionEffect(PotionEffectType.INVISIBILITY, Integer.MAX_VALUE, 0);
        for (Player onlinePlayer : Foxtrot.getInstance().getServer().getOnlinePlayers()) {
