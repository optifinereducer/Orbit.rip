    ...
    public void tick() {
        if (currentCapper != null) {
            ...
            for (Player player : Foxtrot.getInstance().getServer().getOnlinePlayers()) {
                ...
                if (onCap(player.getLocation()) && !player.isDead() && player.getGameMode() == GameMode.SURVIVAL && !ModUtils.isInvisible(player) && !Foxtrot.getInstance().getPvPTimerMap().hasTimer(player.getUniqueId())) {
                    onCap.add(player);
                }
            }
